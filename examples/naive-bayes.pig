...
test_data = LOAD 'naive-bayes.data.training' AS (test_result:int, a:int, b:int, c:int, d:int, e:int, f:int, g:int, h:int, i:int, j:int, k:int, l:int, m:int, n:int, o:int, p:int);
...
result = FOREACH real_data { flds = real_data_aux::fields; spam = test_spam_cnt::data; notspam = test_notspam_cnt::data; result = rfuncs.CalcProb(flds, spam, notspam); GENERATE result.probabilities; }
